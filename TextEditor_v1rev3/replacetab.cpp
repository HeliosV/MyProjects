...
ReplaceTab::ReplaceTab(Window &w, FindWindow &fwnd) : QWidget(), mainwnd(w), findw(fwnd)
{
    setAttribute(Qt::WA_DeleteOnClose);

    QGridLayout *mainLayout = new QGridLayout;

    QLabel *replacelab = new QLabel;
    replacelab->setText("Replace for: ");
    ...
    editfor = new QLineEdit;
    replaceButton = new QPushButton(tr("Replace"));
    connect(replaceButton, SIGNAL(clicked()), this, SLOT(replaceText()));

    replaceLayout->addWidget(replacelab);
    replaceLayout->addWidget(editfor);
    ...
    mainLayout->addLayout(replaceLayout, 1, 0);
    mainLayout->addLayout(settingsLayout, 2, 0);

    setLayout(mainLayout);
}


void ReplaceTab::findNextText()
{
...
void ReplaceTab::replaceText()
{
    if (mainwnd.activeTab())
    {
        mainwnd.activeTab()->setFocus();
        mainwnd.activeTab()->insertPlainText(editfor->text());
        findNextText();
    }
}
